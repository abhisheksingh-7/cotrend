...
import torch
import transformers
import index as faiss_index
from clapt.modeling import CLAPT
import contriever.src.contriever as contriever
import contriever.src.utils as utils
import contriever.src.slurm as slurm
import contriever.src.data as data
from contriever.src.evaluation import calculate_matches
import contriever.src.normalize_text as normalize_text
...
def add_embeddings(indexer, embeddings, ids, indexing_batch_size):
    end_idx = min(indexing_batch_size, embeddings.shape[0])
    ids_toadd = ids[:end_idx]
    embeddings_toadd = embeddings[:end_idx]
    ids = ids[end_idx:]
    embeddings = embeddings[end_idx:]
    indexer.index_data(ids_toadd, embeddings_toadd)
...
def main(args):
    ...
    index_path = os.path.join(embeddings_dir, "faiss_index.faiss")
    if args.save_or_load_index and os.path.exists(index_path):
        indexer.deserialize_from(embeddings_dir)
    else:
        print(f"Indexing passages from files {input_paths}")
        start_time_indexing = time.time()
        index_encoded_data(indexer, input_paths, args.indexing_batch_size)
        print(f"Indexing time: {time.time()-start_time_indexing:.1f} s.")
        if args.save_or_load_index:
            indexer.serialize(embeddings_dir)

    # load passages
    passages = data.load_passages(args.passages)
    ...
    for path in data_paths:
        ...
        top_ids_and_scores = indexer.search_knn(questions_embedding, args.n_docs)
